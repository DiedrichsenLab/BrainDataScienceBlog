...
<section markdown="1">
In neuroimaging, we often want to know how similar, or overlapping, two brain activity patterns are. If two tasks (let's call them *A* and *B*) activate a specific brain region in a similar way, we can infer that this brain region does something similar in the two conditions. Often, we are not so much concerned with whether task *A* activates the region more than task *B* - rather we want to know whether the *pattern* of activation in the region is the same or not - that is, we are interested in the *correlation* between the two activity patterns.
...
To derive this quantity, we need to do a little bit of math. For simplicity let's assume that the two true patterns are identical (and hence perfectly correlated). We further assume that the measure patterns consist of the true pattern and measurement noise
...
By substitution in the previous formula, we can see that the noise ceiling for a correlation between two measured vectors (ie. the value that we should get if the true vectors were perfectly correlated) is the *geometric mean*, i.e. the square-root of the product, of their reliabilities.
...
We can then plug in these unbiased estimators into our correlation formula:
...
I hope that at this point you can see the advantages of using a generative approach (as in PCM) to evaluate the correlations between activity patterns. Before jumping in and using the PCM toolbox, however, it is good to understand a few technical details that happen behind the scenes.
...
Let's denote these hyper-parameters {+side:These unknowns are considered hyper-parameters (rather than parameters), as they characterize the variances and covariances of patterns, rather than the patterns themselves}.
...
If you want to compare models with different numbers of parameters, a more sophisticated approach is required. For example, in Figure 4, we may want to test whether the likelihood for the maximum likelihood estimate for the correlation is larger than the correlation for 1. If we simply take the maximum likelihood for each subjects, the answer would be yes. A slightly different approach would be to use the group-estimate of the best correlation. While this reduced the bias tremendously, it is still a bit biased, as the mean still contains the data from the subject on which we are testing.{+margin:Another approach is to become even "more Bayesian" and treat the parameters as unknown variables with a prior, and then introduce "hyper-hyper parameters" to describe these priors [+citep:Friston2019]}. In this case, we should determine the best-fitting correlation on n-1 subjects, and evaluate the likelihood on the remaining (nth) subject. For more on group fits and cross-validated group fits see the PCM documentation.

## Extensions
The situation considered here with two activity patterns and two measures per activity pattern is quite simple. In most experiments, we have more complicated situations. Often want to know the correlation between patterns associated with specific items observed under two conditions. For example, we want to know how the patterns associated with 4 different movements correlate (on average) across planning and execution. Or how the pattern associated with 30 objects correlated when they are presented as words or in sign language. In these scenarios we need to model the general differences between the 2 conditions (i.e planning vs. execution, or words vs. signs). Additionally, we often need to account for the correlation between patterns in an imaging run, or different co-variance between the patterns for different items. Coming up with a good solution for all these problems using the noise-ceiling or cross-block approach can be very difficult. I find it much easier to add these details into the generative model - and indeed all these situations are implemented in the PCM toolbox.
...
As you can see from the example in Figure 4, using a generative approach is not a magic bullet that suddenly gives you significant results from very noisy data. For example, while we could see that the correlation was larger than $0.6$, we could not distinguish between correlations anywhere between $0.7 - 1.0$. But at least if provides a valid (and in the likelihood sense optimal) evaluation of the real evidence that you have in your data about the true correlation between two activity patterns.
</section>
